...
## 0. 自社コンテキストと意思決定への示唆（ズコーシャ IT事業部 / 事業変革室）

本資料は一般比較に加えて、以下の自社前提を踏まえて読むことを推奨する。

**前提（社内）**
- 建設コンサル ズコーシャ（北海道帯広市）。**道東で唯一の DC を 20 年運営**し、自治体・民間データを受託してシステム運用/導入支援。
- IT事業部 イノベーション課（地理空間情報を戦略技術とした新規サービス開発）＝ **新規事業立ち上げ・収益化が優先**。
- 事業変革室（全社横断・長期）＝ **外部との共創 / コレクティブ・インパクトによる社会課題解決＋事業性の両立**がミッション。

**示唆 1: 自社 DC があるので「セルフホスト型 OSS スタック」が有力な第5の選択肢**
- 構成例: **OSM データ + MapLibre GL JS + 自前ベクタータイル（OpenMapTiles / Martin / Tegola）+ ジオコーダ（Nominatim / Photon）** を自社 DC で運用。
- メリット: **API 課金ゼロ・データ主権を完全保持・自治体データとの統合が容易・ベンダーロックイン回避**。長期/共創ミッションと整合。
- デメリット: タイル生成・更新、ジオコーダ精度チューニングの**運用コスト**。日本の POI / 番地精度は OSM 品質依存で**地方ほど穴**が出やすい。
- 4社比較との関係: 「地図表示・住所変換はセルフホスト、日本の POI 検索だけ Google を従量併用」が現実的なハイブリッド。

**示唆 2: 自治体・民間データ受託 ＝「保存可否・データ主権」が最優先論点**
- §5 の Temporary/Permanent/Stored が直接効く。受託データと外部 API 結果を**自社 DB に保存/突合**する場合:
  - Google: **Place ID 以外は保存不可** → 受託業務での結果保存用途には不向き。
  - Mapbox: 保存するなら **Permanent（$5/1,000・無料枠なし）** 必須。Temporary で保存は ToS 違反。
  - AWS: **intendedUse=Stored（$4/1,000）**。ただし **HERE プロバイダは日本データ保存禁止** → Esri 等の選定 or 要確認。
  - MapTiler / セルフホスト OSM: **検索結果の保存が許可**され、データ主権要件と最も相性が良い。
- 自治体データを扱う案件では、**API 提供国・データ保管リージョン・規約上の保存制限**を要件定義段階で確認すること。

**示唆 3: 道東 / 北海道という地域性 ＝ 地方カバレッジを必ず実地検証**
- 都市部基準の「Google 最強」は地方でも概ね成立するが、**道東の小規模事業者・農地/林地・公共施設**の POI/住所精度は都市部ほど高くない可能性。
- 建設コンサル特有のデータ（**地籍・農地筆・林班・インフラ台帳**など）は商用地図 API には存在しない → **自社 GIS データ + 地図 API のレイヤ重ね**が前提。
- 採用前に帯広・道東の実サンプルで forward/reverse geocoding と POI 検索を必ずテストする。

**示唆 4: 二つのミッションで最適解が分かれる**
- **収益化（IT事業部）**: SaaS スケール時のコストは §7 の通り。BtoG/BtoB で日本 POI 精度が要件なら Google を部分採用し、地図表示はセルフホスト/MapTiler でコストを抑える。
- **共創・社会課題（事業変革室）**: **OSS / オープンデータ寄り（OSM / MapLibre / MapTiler）** が、オープン性・持続性・外部連携と整合。OSM attribution はオープン文脈ではむしろ親和的。

**示唆 5: デプロイ方針との対応（社内基本方針に準拠）**
| 構成要素 | 推奨デプロイ先 |
|---|---|
| 比較資料・静的デモ地図（MapLibre + 静的タイル） | GitHub Pages（public）/ ConoHa WING（private リポジトリ） |
| ジオコーディング/検索 API プロキシ・軽量動的処理 | CloudFlare（Workers） |
| 常駐ジオコーダ（Nominatim / Photon）・タイルサーバ | Railway または**自社 DC** |
| マネージド大規模・既存 AWS 連携（Amazon Location 等） | AWS |
...
| **自社 DC 活用 / データ主権 / 共創** | **セルフホスト OSS（OSM + MapLibre + Nominatim/Photon）**。API 課金ゼロ・データ主権保持・ロックイン回避（運用コストと地方 POI 精度はトレードオフ）。→ **§0 自社コンテキスト参照**。 |
